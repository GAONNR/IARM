...
class PreProcessing():
    ...
    def prepare_data(self, data, batch_id, word_embeddings):
        ...
        aspect_ = self.tokenizer.texts_to_sequences(
            list(self.aspect[limit[0]:limit[1]]))
        train_temp = []
        j = 0
        for datam in data[limit[0]:limit[1]]:
            train_temp.append([datam, aspect_sequence[j], aspect_[
                              j], self.labels[limit[0]:limit[1]][j]])
            j = j+1
        training_data_x0 = []
        training_data_x1 = []
        training_data_y = []
        ...
        for item1 in train_temp:
            ...
            att_tensor = autograd.Variable(torch.FloatTensor(
                att) if not args.cuda else torch.cuda.FloatTensor(att), requires_grad=False)

            temp_mask_sent = att_tensor.view(
                att_tensor.size()[0], -1).expand(-1, 2*EMBEDDING_DIM)
            att_tensor = att_tensor.unsqueeze(0)
            tensor = torch.LongTensor(
                sent) if not args.cuda else torch.cuda.LongTensor(sent)
            sent1 = autograd.Variable(tensor)

            aspects1 = []
            for item in aspects:
                temp = torch.LongTensor(
                    item) if not args.cuda else torch.cuda.LongTensor(item)
            ...
            aspect = torch.LongTensor(
                aspect) if not args.cuda else torch.cuda.LongTensor(aspect)
            ...
            for i in range(0, MAX_ASPECTS-len(aspects)):
                sepr.append(autograd.Variable(torch.zeros(
                    (MAX_SEQUENCE_LENGTH, 2*EMBEDDING_DIM)).type(ftype).unsqueeze(0)))
                att2.append(0)

            for item in aspects1:
                item = item.expand(len(sent), -1)
                sepr.append(
                    torch.mul(torch.cat([embeds, item], dim=1), temp_mask_sent).unsqueeze(0))
                att2.append(1)

            aspect1 = torch.mul(
                torch.cat([embeds, aspect1], dim=1), temp_mask_sent)

            att2_tensor = autograd.Variable(torch.FloatTensor(
                att2) if not args.cuda else torch.cuda.FloatTensor(att2), requires_grad=False).unsqueeze(0)
...
class AttnRNN(nn.Module):
    ...
    def forward(self, sents, aspects, attention_mat1, attention_mat2, batch_size):
        ...
        for sent_asp in sents:
            ...
            output, hidden_sentence_gru = self.sentence_gru(
                embedded, self.hidden_sentence_gru)
            # print(attention_mat1.size())
            temp_attention_mat1 = attention_mat1.view(attention_mat1.size(
            )[0], attention_mat1.size()[1], 1).expand(-1, -1, output.size()[2])
           ...
           # sys.exit(1)
            ...
            # print(attention_mat1.size())
            # sys.exit(1)
            masked_attn_weights = torch.mul(
                attn_weights.squeeze().permute(1, 0), attention_mat1)
            # print(masked_attn_weights.size())
            _sums = masked_attn_weights.sum(-1).unsqueeze(
                1).expand(-1, masked_attn_weights.size()[1])
            # print(_sums.size())
            attentions = masked_attn_weights.div(
                _sums).unsqueeze(1).permute(2, 0, 1)
        ...
        output, hidden_aspect_gru = self.aspect_gru(
            aspec_rep, self.hidden_aspect_gru)

        temp_attention_mat2 = attention_mat2.view(attention_mat2.size(
        )[0], attention_mat2.size()[1], 1).expand(-1, -1, output.size()[2])
        ...
        temp_attention_mat3 = attention_mat1.view(attention_mat1.size(
        )[0], attention_mat1.size()[1], 1).expand(-1, -1, outputa_.size()[2])
        ...
        masked_attn_weights_ = torch.mul(
            attn_weights_.squeeze().permute(1, 0), attention_mat1)
        _sums_ = masked_attn_weights_.sum(-1).unsqueeze(
            1).expand(-1, masked_attn_weights_.size()[1])
        attentions_ = masked_attn_weights_.div(
            _sums_).unsqueeze(1).permute(2, 0, 1)
        ...
        for i in range(0, self.hop_size):
            ...
            output_w, hidden_aspect_write_gru = \
                self.aspect_write_gru(output.permute(
                    2, 0, 1), self.hidden_aspect_write_gru)

            output_w = torch.mul(output_w.permute(
                1, 0, 2), temp_attention_mat2)
            output_w = self.dropout2(output_w)

            masked_attn_weights2 = torch.mul(
                attn_weights2.squeeze().permute(1, 0), attention_mat2)
            # print(masked_attn_weights.size())
            _sums2 = masked_attn_weights2.sum(-1).unsqueeze(
                1).expand(-1, masked_attn_weights2.size()[1])
            # print(_sums.size())
            attentions2 = masked_attn_weights2.div(
                _sums2).unsqueeze(1).permute(2, 0, 1)
            ...
            attn_applied = torch.bmm(attentions2.permute(
                1, 2, 0), output_w.permute(0, 1, 2)).squeeze()
...
def train(onea):
    ...
    we = Glove(GLOVE_DIR=options.GLOVE_DIR)
    ei = index_word_embeddings(tokenizer.word_index, we)
    word_embeddings = nn.Embedding(
        len(tokenizer.word_index)+1, EMBEDDING_DIM, padding_idx=0)
    word_embeddings.weight = nn.Parameter(torch.FloatTensor(
        ei) if not args.cuda else torch.cuda.FloatTensor(ei))
    word_embeddings.weight.requires_grad = False
    print("Embeddings loaded....")

    model = AttnRNN(HOP_SIZE, BATCH_SIZE, EMBEDDING_DIM,
    ...
    for i in range(NB_EPOCH):
        ...
        for batch_id in range(batch_count):
            optimizer.zero_grad()
            bdata_x0, bdata_x1, bdata_y, attention_mat2, attention_mat1 = prep.prepare_data(
                data, batch_id, word_embeddings)
            model.hidden_sentence_gru = model.init_hidden2(bdata_x0.size()[0])
            model.hidden_aspect_gru = model.init_hidden(bdata_x0.size()[0])
            model.hidden_aspect_write_gru = model.init_hidden(
                bdata_x0.size()[0])
            #model.hidden_aspect_repr_gru = model.init_aspect_hidden(bdata_x0.size()[0])

            prediction, _, _ = model(
                bdata_x0, bdata_x1, attention_mat1, attention_mat2, bdata_x0.size()[0])
        ...
        print('Epoch %d train_loss %.4f train_acc %.2f test_loss %.4f test_acc %.2f time %.2f' % (
            i+1, np.mean(loss_tot), accuracy(pred_res, true_label), test_loss, accuracy(preds, true), time.time()-start_time))
        # import ipdb;ipdb.set_trace()
        mul = set(range(len(true)))-set(onea)
        print('single_aspect %.2f mul_aspect %.2f' % (accuracy([preds[idx] for idx in onea], [
              true[idx] for idx in onea]), accuracy([preds[idx] for idx in mul], [true[idx] for idx in mul])))
...
def csv_reader(file):
    ...
    with open(file, 'rb') as csvfile:
        ...
        for row in aspectreader:
            ...
            aspects = [x.replace("'", "").replace('[', "").replace("\"", "").replace(
                ']', "").strip().lower() for x in row[2].split(",")]
            sentiments = [x.strip().replace("'", "").replace('[', "").replace(
                "\"", "").replace(']', "").lower() for x in row[3].split(",")]
